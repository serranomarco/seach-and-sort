function binarySearch(list, target) {
  ...
  const lowArr = list.slice(0, slicePoint);
  const highArr = list.slice(slicePoint + 1);

  if (target === list[slicePoint]) return true;

  if (target < list[slicePoint]) {
    return binarySearch(lowArr, target);
  } else {
    return binarySearch(highArr, target);
  }
}
...
function binarySearchIndex(list, target, low = 0, high = list.length - 1) {
  ...
  const slicePoint = Math.floor((high + low) / 2);
  if (target < list[slicePoint]) {
